        ...
        if (vht_cfg->txrx & MLAN_RADIO_RX)
        {
            ...
            else
            {
                ...
                vht_cfg->vht_rx_max_rate =
                    wlan_convert_mcsmap_to_maxrate(pmpriv, (mlan_band_def)vht_cfg->band, (t_u16)vht_cfg->vht_rx_mcs);
                vht_cfg->vht_tx_max_rate =
                    wlan_convert_mcsmap_to_maxrate(pmpriv, (mlan_band_def)vht_cfg->band, (t_u16)vht_cfg->vht_tx_mcs);
            }
            LEAVE();
            return ret;
        }
