...
int wrapper_wifi_assoc(const unsigned char *bssid, int wlan_security, bool is_wpa_tkip, unsigned int owe_trans_mode)
{
    ...
    /* if (!bssDesc) */
    /* 	return -WM_FAIL; */
    int idx = wlan_find_bssid_in_list(priv, (unsigned char *)bssid, (mlan_bss_mode)-1);
    if (idx == -1)
    {
    ...
    if (wlan_11d_support_is_enabled(priv))
    {
        ...
        {
            if (priv->support_11d->wlan_11d_create_dnld_countryinfo_p(priv, (mlan_band_def)d->bss_band) !=
                MLAN_STATUS_SUCCESS)
            {
...
{
    mlan_private *pmpriv = (mlan_private *)mlan_adap->priv[0];
    mlan_status rv       = wlan_set_regiontable(pmpriv, region, (mlan_band_def)band);
    if (rv != MLAN_STATUS_SUCCESS)
    {
        return -WM_FAIL;
    }
