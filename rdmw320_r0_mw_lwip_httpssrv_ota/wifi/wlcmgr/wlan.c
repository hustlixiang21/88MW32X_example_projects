...
static void wlcm_process_net_if_config_event(struct wifi_message *msg, enum cm_sta_state *next)
{
    ...
    *next = CM_STA_IDLE;
...
#if defined(SD8801)
    /*8801 does not support 40MHz.*/
    httxcfg   = 0x2C; // TODO need to add defines for this setting
    htcapinfo = 0x0800000;
#else
    httxcfg                       = 0x6E; // TODO need to add defines for this setting
    htcapinfo                     = 0x1820000;
#endif
    ...
    ret = wlan_set_htcapinfo(htcapinfo);
    if (ret != WM_SUCCESS)
    {
